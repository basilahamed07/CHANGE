(() => {
  ...
  function fillRichText(editorEl, value) {
    try {
      ...
      console.warn('[jobagent] fillRichText failed:', err.message);
      return false;
    }
  }
  ...
  function showUploadHelper(fileInput, type) {
    ...
    text.textContent = `${label} ready -- download from jobagent, then upload here`;
    ...
    btn.addEventListener('click', async (e) => {
      ...
      if (!currentJobId) {
        text.textContent = 'No job ID available. Open this page from jobagent first.';
        return;
      }
  ...
  async function getNewMappings() {
    try {
      ...
      console.warn('[jobagent] Re-analysis failed:', err?.message || err);
    }
    return null;
  }
  ...
  function createOverlay() {
    ...
    overlayEl.innerHTML = `
      <div class="${PREFIX}-overlay-header">
        <span class="${PREFIX}-overlay-title">jobagent</span>
    ...
    promptEl.innerHTML = `
      <div class="${PREFIX}-learn-modal">
        <div class="${PREFIX}-learn-header">
          <h3 class="${PREFIX}-learn-title">Save ${newData.length} new answer${newData.length > 1 ? 's' : ''} to jobagent?</h3>
  ...
  async function autoTrackApplied() {
    ...
    try {
      ...
      if (result && result.ok) {
        showToast('Job marked as applied in jobagent', 'success');
      }
    } catch (err) {
      console.warn('[jobagent] autoTrackApplied failed:', err.message);
    }
  }
  ...
  function handleSubmission() {
    try {
      ...
      console.warn('[jobagent] handleSubmission failed:', err.message);
    }
  }
  ...
  async function applyCustomQA(mappings) {
    ...
    try {
      ...
      console.warn('[jobagent] applyCustomQA failed:', err.message);
      return mappings;
    }
      ...
      await withTimeout((async () => {
        ...
        if (atsAdapter?.getExtraFields) {
          try {
            ...
            console.warn('[jobagent] ATS getExtraFields failed:', err.message);
          }
        }
        ...
        try {
          ...
          console.warn('[jobagent] enrichFieldHints failed:', err.message);
        }
        ...
        if (atsAdapter?.enhanceExtraction) {
          try {
            ...
            console.warn('[jobagent] ATS enhanceExtraction failed:', err.message);
          }
        }

        // Debug: log extracted fields so we can diagnose fill issues
        console.log('[jobagent] Extracted fields:', structuredFields.map(f => ({
        ...
        console.log('[jobagent] Analyze response:', JSON.stringify(response?.data?.mappings || [], null, 2));
  ...
  function showBadge(confidence) {
    ...
    badgeEl.querySelector('.cp-auto-badge-dismiss').addEventListener('click', async (e) => {
      ...
      try {
        ...
        console.warn('[jobagent] Failed to save dismissed host:', err.message);
      }
      removeBadge();
    });
  }
  ...
  async function tryShowBadge() {
    ...
    try {
      ...
      console.warn('[jobagent] Failed to check dismissed hosts:', err.message);
    }
  }
  ...
  function createSaveButton(jobData, card) {
    ...
    btn.title = 'Save this job to jobagent';
  ...
  function showScoreBadge(card, score) {
    ...
    badge.title = `jobagent match score: ${numScore}%`;
