...
@pytest.mark.asyncio
async def test_feature_flag_gating(tmp_path):
    """D22: US-specific endpoints return 404 when their feature flag is off."""
    from app.main import create_app
    from app.config import Settings
    application = create_app(db_path=str(tmp_path / "flag.db"), testing=True)
    application.state.settings = Settings(_env_file=None)  # all flags default off
    db = Database(str(tmp_path / "flag.db"))
    await db.init()
    application.state.db = db
    transport = ASGITransport(app=application)
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        assert (await ac.get("/api/offers")).status_code == 404
        assert (await ac.post("/api/career/analyze")).status_code == 404
        r = await ac.get("/api/jobs/1/predict-success")
...
async def app(tmp_path):
    ...
    # These tests build the app without running lifespan; routers now read
    # app.state.settings for feature flags. Flags are enabled here because these
    # tests exercise endpoint mechanics, not flag behaviour (flag-off behaviour
    # is covered by test_feature_flag_gating below).
    application.state.settings = Settings(
        enable_salary_tools=True,
