...
async def system_health(request: Request):
    """Phase 32 / M1: system health alias with product metadata.

    Calls the same logic as GET /api/health — one service layer, two URLs.
    """
    base = await health(request)  # dict on healthy, Response(503) on unhealthy
    if isinstance(base, Response):
        return base
    body = dict(base)
    body["product"] = "jobagent"
    body["version"] = "0.1.0"
    body["features"] = {
        "salary_tools": request.app.state.settings.enable_salary_tools,
        "career_advisor": request.app.state.settings.enable_career_advisor,
        "success_predictor": request.app.state.settings.enable_success_predictor,
        "extension_autofill": request.app.state.settings.enable_extension_autofill,
        "linkedin_guest": request.app.state.settings.enable_linkedin_guest,
        "allow_send": request.app.state.settings.allow_send,
    }
    return body
