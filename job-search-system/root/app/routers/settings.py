...
def _looks_like_ollama_url(url: str) -> bool:
    """Detect Ollama-local URLs that must not be saved as hosted-provider base_url."""
    u = (url or "").lower()
    return "localhost:11434" in u or "127.0.0.1:11434" in u


# Known key shapes — used to catch pasted prose masquerading as a credential.
_KEY_PREFIXES = {
    "openrouter": "sk-or-",
    "openai": "sk-",
    "anthropic": "sk-ant-",
...
def _validate_key_shape(provider: str, api_key: str) -> None:
    prefix = _KEY_PREFIXES.get(provider)
    if prefix and api_key and not api_key.startswith("****") and not api_key.startswith(prefix):
        raise HTTPException(
            400,
            f"That doesn't look like a {provider} API key — {provider} keys start "
            f"with '{prefix}'. Check that you copied the key itself, not other text.",
        )


@router.get("/profile")
async def get_profile(request: Request):
    profile = await _db(request).get_user_profile()
...
async def delete_custom_qa(request: Request, qa_id: int):
    await _db(request).delete_custom_qa(qa_id)
    return {"ok": True}


@router.get("/autofill/history")
async def get_autofill_history(request: Request, limit: int = Query(50)):
    return {"items": await _db(request).get_autofill_history(limit=limit)}
...
async def delete_work_history(request: Request, entry_id: int):
    await _db(request).delete_work_history(entry_id)
    return {"ok": True}
...
async def delete_education(request: Request, entry_id: int):
    await _db(request).delete_education(entry_id)
    return {"ok": True}
...
async def delete_certification(request: Request, entry_id: int):
    await _db(request).delete_certification(entry_id)
    return {"ok": True}
...
async def delete_skill(request: Request, entry_id: int):
    await _db(request).delete_skill(entry_id)
    return {"ok": True}
...
async def delete_language(request: Request, entry_id: int):
    await _db(request).delete_language(entry_id)
    return {"ok": True}
...
async def delete_reference(request: Request, entry_id: int):
    await _db(request).delete_reference(entry_id)
    return {"ok": True}


@router.get("/resumes")
async def list_resumes(request: Request):
    resumes = await _db(request).get_resumes()
...
async def delete_resume(request: Request, resume_id: int):
    deleted = await _db(request).delete_resume(resume_id)
    if not deleted:
        raise HTTPException(404, "Resume not found")
    return {"ok": True}
...
async def set_default_resume(request: Request, resume_id: int):
    result = await _db(request).set_default_resume(resume_id)
    if not result:
        raise HTTPException(404, "Resume not found")
    return {"ok": True}
...
async def list_saved_views(request: Request):
    views = await _db(request).get_saved_views()
    return {"views": views}
...
async def delete_saved_view(request: Request, view_id: int):
    deleted = await _db(request).delete_saved_view(view_id)
    if not deleted:
        raise HTTPException(404, "View not found")
    return {"ok": True}
...
async def get_search_config(request: Request):
    config = await _db(request).get_search_config()
    if not config:
...
async def update_search_terms(request: Request):
    ...
    await _db(request).update_search_terms(terms)
...
async def update_remote_only(request: Request):
    ...
    await _db(request).set_remote_only(enabled)
    return {"ok": True, "remote_only": enabled}


@router.get("/ai-settings")
async def get_ai_settings(request: Request):
    settings = await _db(request).get_ai_settings()
    if not settings:
...
async def update_ai_settings(request: Request):
    ...
    # --- credential hygiene (guards against corrupted pastes) ---------------
    # Non-ASCII characters (e.g. a '✓' copied alongside the key) crash HTTP
    # header encoding later; keys never contain whitespace.
    api_key = "".join(ch for ch in (api_key or "") if ch.isascii() and not ch.isspace())
    model = "".join(ch for ch in (model or "") if ch.isascii())
    # A base_url is only meaningful for self-hosted providers; an Ollama URL on
    # a hosted provider silently redirects every AI call to localhost.
    if provider != "ollama" and _looks_like_ollama_url(base_url):
        base_url = ""
    _validate_key_shape(provider, api_key)
    ...
    if api_key.startswith("****") or (provider == "bedrock" and base_url.startswith("****")):
        existing = await _db(request).get_ai_settings()
    ...
    await _db(request).save_ai_settings(provider, api_key, model, base_url, region=region)
...
async def test_ai_connection(request: Request):
    ...
    base_url = body.get("base_url", "")
    region = body.get("region", "")
    # Same credential hygiene as update_ai_settings.
    api_key = "".join(ch for ch in (api_key or "") if ch.isascii() and not ch.isspace())
    model = "".join(ch for ch in (model or "") if ch.isascii())
    if provider != "ollama" and _looks_like_ollama_url(base_url):
        base_url = ""
...
async def get_embedding_settings(request: Request):
    settings = await _db(request).get_embedding_settings()
    if not settings:
        return {
...
async def save_embedding_settings(request: Request):
    ...
    if api_key.startswith("****"):
        existing = await _db(request).get_embedding_settings()
        if existing:
            api_key = existing["api_key"]
    db = _db(request)
...
async def test_email_settings(request: Request):
    ...
    data = await request.json()
    existing = await _db(request).get_email_settings()
    if data.get("smtp_password") == "" and existing:
        data["smtp_password"] = existing.get("smtp_password", "")
    ...
    success = await send_email(
        data, to=test_to, subject="jobagent SMTP Test",
...
async def get_scraper_schedule(request: Request):
    schedules = await _db(request).get_all_scraper_schedules()
...
async def update_scraper_schedule(request: Request):
    ...
    await _db(request).update_scraper_schedule(source_name, int(interval_hours))
    return {"ok": True}
...
async def upload_resume(request: Request, file: UploadFile = File(...)):
    ...
    if filename.endswith(".pdf"):
        ...
        # DOCX is a ZIP container — decode it properly instead of storing raw bytes.
        import io
        import docx
        ...
        if not resume_text.strip():
            raise HTTPException(400, "The .docx file contains no readable text.")
    else:
