...
async def list_jobs(
    ...
    limit: int = Query(50),
    offset: int = Query(0),
    # Frontend always sends min_score (possibly as an empty string). Declaring
    # it int caused a 422 for every default Jobs-page load — an empty string
    # must be treated as "no filter", not a validation error.
    ...
    try:
        min_score_val = int(min_score) if min_score and str(min_score).strip() else None
    ...
    jobs = await db.list_jobs(
        ...
        min_score=min_score_val, search=search, source=source,
