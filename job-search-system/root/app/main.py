...
def _build_ai_client(ai_settings: dict | None, settings=None, env_key: str = "") -> AIClient | None:
    """Build an AIClient from DB settings or env fallback.

    Env fallback order: OpenRouter (JOBAGENT_OPENROUTER_API_KEY, product default),
    then Anthropic (JOBAGENT_ANTHROPIC_API_KEY), then legacy env_key parameter.
    """
...
async def lifespan(app: FastAPI):
    ...
    if not testing:
        ...
        client = _build_ai_client(ai_settings, settings=settings)
        ...
        try:
            ...
            logger.info(
                "Evidence store: %d claims (%d verified), %d synced to DB",
                len(store.claims), len(store.verified_values()), synced,
            )
        except Exception:
            logger.exception("Evidence store init failed — generation gate will fail closed")
            app.state.evidence_store = None
            app.state.evidence_checker = None

        # M3: country strategy registry (config/countries/*.yaml — zero-code country
        # addition) + sync alias/city terms into the location classifier.
        from app.country_registry import CountryRegistry
        from app.location_classifier import set_extra_country_terms
        try:
            registry = CountryRegistry(settings.countries_dir).load()
            app.state.country_registry = registry
            set_extra_country_terms(registry.classification_terms())
            app.state.location_classifier_terms = registry.classification_terms()
            logger.info("Country strategy: %d countries (%s)",
                        len(registry.countries),
                        ", ".join(registry.enabled_region_names()) or "none enabled")
        except Exception:
