...
class Database:
    ...
    async def _create_tables(self):
        await self.db.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS job_duplicates (
                ...
                UNIQUE(job_id, duplicate_job_id, signal),
                FOREIGN KEY (job_id) REFERENCES jobs(id) ON DELETE CASCADE,
    ...
    async def _migrate(self):
        ...
        jobs_migrations = {
            ...
            # M3: WHY a job was dismissed. 'country_strategy' = auto-dismissed by
            # the strategy engine (reversible when strategy widens); NULL = not
            # strategy-dismissed (user dismiss / other reasons stay permanent).
            "strategy_dismissed": "ALTER TABLE jobs ADD COLUMN strategy_dismissed INTEGER DEFAULT 0",
        ...
        """)
        await self.db.execute("""            CREATE TABLE IF NOT EXISTS ical_tokens (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
...
""")
        ...
        # Candidate evidence store (M2): one row per candidate claim with its
        # verification status. Synced from data/profile/*.yaml by EvidenceStore.
        await self.db.execute("""
            CREATE TABLE IF NOT EXISTS candidate_evidence (
                ...
                status TEXT NOT NULL DEFAULT 'UNVERIFIED'
                    CHECK (status IN ('VERIFIED', 'UNVERIFIED', 'DISPUTED', 'DO_NOT_USE')),
                source TEXT,
                last_verified TEXT,
                notes TEXT,
                synced_at TEXT NOT NULL,
                UNIQUE(category, claim_id)
            )
""")
    ...
    async def get_candidate_evidence(self) -> list[dict]:
        ...
        cursor = await self.db.execute(
            "SELECT category, claim_id, value, status, source, last_verified, notes, synced_at "
            "FROM candidate_evidence ORDER BY category, claim_id"
        )
        rows = await cursor.fetchall()
        return [
            {
    ...
    async def record_freshness(self, job_id: int, evidence: dict) -> None:
        """M5: persist a freshness assessment on the job (audit trail)."""
        await self.db.execute(
            "UPDATE jobs SET freshness_evidence = ? WHERE id = ?",
            (json.dumps(evidence), job_id))
        await self.db.commit()

    async def add_duplicate_link(self, job_id: int, duplicate_job_id: int,
                                 signal: str) -> bool:
        """M5: record a repost/duplicate edge in the job_duplicates graph.

        Idempotent per (job_id, duplicate_job_id, signal). Returns True if new.
        """
        if job_id == duplicate_job_id:
            return False
        cursor = await self.db.execute(
            """INSERT OR IGNORE INTO job_duplicates
               (job_id, duplicate_job_id, signal, detected_at)
               VALUES (?, ?, ?, ?)""",
            (job_id, duplicate_job_id, signal,
             datetime.now(timezone.utc).isoformat()))
        await self.db.commit()
        return cursor.rowcount > 0

    async def get_duplicate_links(self, job_id: int) -> list[dict]:
        cursor = await self.db.execute(
            """SELECT jd.*, j.title, j.company, j.url, j.dismissed
               FROM job_duplicates jd JOIN jobs j ON jd.duplicate_job_id = j.id
               WHERE jd.job_id = ?""", (job_id,))
        rows = await cursor.fetchall()
        out = [dict(r) for r in rows]
        cursor = await self.db.execute(
            """SELECT jd.*, j.title, j.company, j.url, j.dismissed
               FROM job_duplicates jd JOIN jobs j ON jd.job_id = j.id
               WHERE jd.duplicate_job_id = ?""", (job_id,))
        rows2 = await cursor.fetchall()
        out += [dict(r) for r in rows2]
        return out

    async def get_eligible_jobs(self, limit: int = 500) -> list[dict]:
        """M5: jobs eligible for the application pipeline (deterministic gates).

        DATE_UNKNOWN jobs are EXCLUDED (Critical Test #3 — never counted toward
        the daily target). Stale and dismissed jobs excluded. Uses the same
        freshness limit as assess_freshness (7 days, Golden Rule 10).
        """
        ...
        cursor = await self.db.execute(
            """SELECT * FROM jobs
               ...
               AND location_region IN (SELECT json_each.value
                   FROM search_config, json_each(search_config.allowed_regions))
               AND posted_date IS NOT NULL
               AND posted_date >= ?
               AND LENGTH(COALESCE(description, '')) >= 80
    ...
    async def dismiss_jobs_outside_regions(self, allowed_regions: list[str]) -> int:
        ...
        M3: strategy dismissals are REVERSIBLE — flagged via strategy_dismissed=1
        so a later, wider strategy can restore them. User dismissals (plain
        dismissed=1) are never touched.
        """
        if not allowed_regions:
            return 0
        placeholders = ",".join("?" for _ in allowed_regions)
        cursor = await self.db.execute(
            f"""UPDATE jobs SET dismissed = 1, strategy_dismissed = 1
                WHERE dismissed = 0
                AND location_classified = 1
    ...
    async def restore_strategy_dismissed_jobs(self) -> int:
        """M3: re-admit jobs a PREVIOUS, narrower strategy auto-dismissed.

        Only strategy_dismissed=1 rows are restored (user dismissals stay
        permanent), and only jobs with no active application are affected.
        """
        cursor = await self.db.execute(
            """UPDATE jobs SET dismissed = 0, strategy_dismissed = 0
               WHERE strategy_dismissed = 1 AND dismissed = 1
               AND id NOT IN (
                   SELECT job_id FROM applications WHERE status != 'interested'
               )"""
        )
    ...
    async def update_allowed_regions(self, regions: list[str]):
        now = datetime.now(timezone.utc).isoformat()
        # Upsert: a fresh DB has no search_config row yet — a bare UPDATE would
        # silently match nothing and strategy sync would be lost (found by the
        # M4 cross-source dedup test).
        await self.db.execute(
            """INSERT INTO search_config (id, allowed_regions, updated_at)
               VALUES (1, ?, ?)
               ON CONFLICT(id) DO UPDATE SET
