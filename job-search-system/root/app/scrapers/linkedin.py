...
class LinkedInScraper(BaseScraper):
    ...
    def _build_params(self, query: str, start: int = 0) -> dict:
        return {
            "keywords": query,
            # Empty location = worldwide/remote results. The M3 country strategy
            # engine will pass explicit target countries here.
            "location": "",
    ...
    async def scrape(self) -> list[JobListing]:
        from app.scrapers.defaults import SEARCH_TERMS as AI_DEFAULT_TERMS
        queries = self.search_terms[:10] if self.search_terms else AI_DEFAULT_TERMS[:10]
        all_jobs = []
        seen_urls = set()
