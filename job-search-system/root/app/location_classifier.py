...
# Registry-driven alias/city terms (M3): loaded from config/countries/*.yaml at
# app startup and refreshed on POST /api/countries/reload. Kept separate so the
# static map above stays untouched — YAML is the source of truth for strategy.
_EXTRA_COUNTRY_TERMS: dict[str, str] = {}


def set_extra_country_terms(terms: dict[str, str]) -> None:
    """Replace registry-driven alias/city terms (word-boundary matched)."""
    global _EXTRA_COUNTRY_TERMS
    _EXTRA_COUNTRY_TERMS = {str(k).lower(): str(v) for k, v in (terms or {}).items()}


def _country_map() -> dict[str, str]:
    return {**_NON_US_COUNTRIES, **_EXTRA_COUNTRY_TERMS}


def _match_country(text: str, country_map: dict[str, str]) -> str | None:
    """Word-boundary country/city match on comma/dash/slash-split parts.

    Replaces bare substring matching, which misclassified 'Remote - Ukraine'
    as UK because 'uk' is contained in 'ukraine'.
    """
    parts = [p.strip() for p in re.split(r"[,\-/|]", text)]
    for part in parts:
        if part in country_map:
            return country_map[part]
    for pattern, country in sorted(country_map.items(), key=lambda kv: -len(kv[0])):
...
def classify_location_rule_based(location: str) -> str | None:
    ...
    for kw in _REMOTE_KEYWORDS:
        if kw in loc_lower:
            ...
            if rest:
                # Check if the rest is a non-US country/city (word-boundary match —
                # substring matching misclassified 'Remote - Ukraine' as UK via 'uk')
                country = _match_country(rest, _country_map())
                if country:
                    return country
    ...
    # Check non-US countries/cities (word-boundary; includes registry-driven terms)
    country = _match_country(loc_lower, _country_map())
    if country:
        return country
